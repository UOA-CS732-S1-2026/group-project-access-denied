/**
 * ThreadVault seed function
 *
 * Called on first login with a per-user mongoose db handle.
 * Populates:
 ...
 *
 * Idempotent — checks before inserting, safe to call multiple times.
 */
...
async function seed(db) {
  // Bind models to the per-user database connection
  ...
  if (!aliceOrderExists) {
    await Order.create({
      ...
      status: 'delivered',
      shippingAddress: { fullName: 'Alice Smith', street: '12 Harbour View', city: 'Auckland', postcode: '1010', country: 'New Zealand' },
  ...
  if (reviewCount === 0) {
    await Review.insertMany([
      { user: admin._id, product: products[0]._id, username: 'admin', rating: 5, body: 'Absolutely stunning piece. The silk drapes beautifully — worth every penny.' },
      { user: alice._id, product: products[0]._id, username: 'alice', rating: 4, body: 'Gorgeous blouse, runs slightly small. Size up if between sizes.' },
      { user: admin._id, product: products[2]._id, username: 'admin', rating: 5, body: 'The construction quality is exceptional. This is a coat you keep for life.' },
    ]);
  }
